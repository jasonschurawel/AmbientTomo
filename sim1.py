...
# -*- coding: utf-8 -*-
"""
Seismische 2D-Wellensimulation - Ausführungsskript (sim1.py)
------------------------------------------------------------
Dieses Skript steuert den gesamten Workflow. Es nutzt rein metrische Einheiten,
berechnet die physikalischen Gitter-Eigenschaften und entkoppelt die Physik
vollständig vom Video-Rendering über eine transiente PKL-Zwischendatei.
"""
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(skript_ordner, "tmp")
    ...
    # =========================================================================
    fps = 60                 # Bilder pro Sekunde im Video (Default: 60)
    zeitlupe_faktor = 6.0    # Zeitdehnung (6.0 bedeutet: 2.5s Physik -> 15s Video)
    ...
    # =========================================================================
    # Die kürzeste Wellenlänge entsteht rein physikalisch bei der niedrigsten 
    # Geschwindigkeit (v_oben) und der höchsten Frequenz (f_max_effektiv)!
    ...
    # Subsampling-Intervall berechnen
    ...
    print(f"\n=========================================================================")
    print(f"PROFI-GEOPHYSIK CONFIGURATION [ID: {naechste_id:04d}]")
    print(f"=========================================================================")
    ...
    print(f"[*] Daten-Pipeline:          Exportiere jeden {speicher_schritt}. Schritt nach PKL.")
    print(f"[*] Video-Spezifikation:     {video_dauer_sec:.1f}s Dauer bei {fps} FPS (= {total_video_frames} Frames)")
    print(f"[*] Speicherpfad PKL:        {pkl_filename}")
    print(f"=========================================================================\n")
    ...
    with open(pkl_filename, 'rb') as f_pkl:
        ...
        print(f"[+] Rendere Videofile -> {video_filename}")
        frame_zaehler = 0
        ...
        with writer.saving(fig, video_filename, dpi=110):
            ...
            try:
                while True:
                    ...
                    ax.set_title(f"Seismische Wellenfront - Zeit: {aktuelle_zeit_s:.3f} s (Zeitlupe: {zeitlupe_faktor}x)")
    ...
    print(f"[+] Video erfolgreich exportiert. Die Simulationsdaten bleiben in '{pkl_filename}' für spätere Analysen erhalten!")
    plt.close()